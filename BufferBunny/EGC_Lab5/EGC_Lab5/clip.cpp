...
namespace egc {
    ...
    vec3 calculateNormal(vec3 p1, vec3 p2)
    {
        float dx = p2.x - p1.x;
        float dy = p2.y - p1.y;
        vec3 normal = vec3(-dy, dx, 0);
        normal = normal.normalize();
        return normal;
    }

    std::vector<vec3> calculateNormals(std::vector<vec3> points)
    {
    ...
    {
        std::vector<vec3> normals = calculateNormals(points);
    ...
    int lineClip_CyrusBeck(std::vector<vec3> clipWindow, vec3& p1, vec3& p2)
    {
        ...
        std::vector<vec3> normals = calculateNormals(clipWindow);
        float tE = 0;
        float tL = 1;
        float t;
        for (int i = 0; i < normals.size(); i++) {
            vec3 D = p2 - p1;
            float dotProd = dotProduct(normals[i], D);
            if (dotProd != 0)
            ...
            vec3 newP1 = p2*tE + p1*(1 - tL);
            vec3 newP2 = p2*tE + p1*(1 - tL);
